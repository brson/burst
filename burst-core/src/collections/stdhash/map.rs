...
use core::hash::{Hash, SipHasher, BuildHasher};
...
impl RandomState {
    ...
    pub fn new() -> RandomState {
        // FIXME: huonw's patch puts keys in a thread local and
        // I don't understand why

        // re https://github.com/rust-lang/rust/pull/31356
        // get some random bytes from the OS
        let mut bytes = [0_u8; 16];
        rand::fill_bytes(&mut bytes);

        // FIXME: weird transmute
        let keys: (u64, u64) = unsafe { mem::transmute(bytes) };

        RandomState { k0: keys.0, k1: keys.1 }
    }
}
